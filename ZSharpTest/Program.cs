...
if (true)
{
    ...
    ZSInternalFunction impl = new(
        ...
        new RTFunctionType(interpreter.LoadIR<ZSSignature>(@internal.Signature))
    );
...
if (true) {
    ...
    compiler.DefineGlobal(
        ...
        new RTFunction(importFunction, interpreter.LoadIR<ZSSignature>(sig))
        );
}
...
if (true)
{
    ...
    var zsSig = interpreter.LoadIR<ZSSignature>(sig);

    getattrFunction.Body.StackSize = 3;
    compiler.DefineCTGlobal(
        desugarer.Context.GetBinaryOperator("."), 
        new CGFunction(getattrFunction, zsSig)
        );
    compiler.DefineRTGlobal(
        desugarer.Context.GetBinaryOperator("."), 
        new CGFunction(getattrFunction, zsSig)
        );
}
...
var rastNodes = new RStatement[]
{
    ...
    new RImport(stdIO)
    {
        Target = new RObjectPattern(new()
        {
            { "print", new RNamePattern(print = new("print")) }
        })
    },
