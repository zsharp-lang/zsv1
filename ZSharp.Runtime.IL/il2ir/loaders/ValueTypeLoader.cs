...
using ValueType = ZSharp.IR.ValueType;

namespace ZSharp.Runtime.NET.IL2IR
{
    internal sealed class ValueTypeLoader(ILLoader loader, Type input)
        : BaseILLoader<Type, ValueType>(loader, input, new(input.Name))
    {
        private OOPTypeReference<ValueType> Self { get; set; }

        public override ValueType Load()
        {
            if (Context.Cache<ValueType>(Input, out var result))
                return result;

            Context.Cache(Input, Output);

            Self = new ValueTypeReference(Output);
