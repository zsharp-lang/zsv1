...
{
    partial class ConstructorCompiler
    {
        ...
        public void CompileSignature()
        {
            ...
            foreach (var param in Node.Signature.Args ?? [])
                ...
                else Object.signature.parameters.Add(co!);

            //foreach (var param in Node.Signature.KwArgs ?? [])
            //    if (Compile(param).When(out var co).Error(out var error))
            //        Interpreter.Log.Error(error.ToString()!, param);
            //else Object.signature.KwArgs.Add(co!);
        }
        ...
        private IResult<Objects.Parameter, Error> Compile(AST.Parameter parameter)
        {
            if (parameter.Type is null)
                return Result<Objects.Parameter>.Error($"Parameter {parameter.Name} must have a type.");

            if (
                CompileExpression(parameter.Type)
                .When(out var type)
                .Error(out var error)
            ) return Result<Objects.Parameter>.Error(error);

            return Result<Objects.Parameter>.Ok(
                new()
                {
                    Name = parameter.Name, 
                    Type = type!
