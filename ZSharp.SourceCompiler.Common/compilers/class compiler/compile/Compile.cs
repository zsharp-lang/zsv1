    ...
    partial class ClassCompiler
    {
        public IResult Declare()
        {
            ...
            if (Node.Name != string.Empty)
                if (!Interpreter
                    ...
                    errors.Append($"Could not bind class {Node.Name}: {error}");

            if (Node.Bases is not null)
                foreach (var baseExpression in Node.Bases)
                    if (
                        Compile(baseExpression)
                        .When(out var @base)
                        .Error(out error)
                    )
                        errors.Append($"Could not bind base class {baseExpression}: {error}");
                    else Spec.Bases.Add(@base!);
        ...
        private IResult Compile(AST.Statement statement)
            => statement switch
            {
                ...
                AST.LetStatement letStatement => Compile(letStatement),
